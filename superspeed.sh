...
speedLog="${SCRIPT_DIR}/speedtest.log"
log="${SCRIPT_DIR}/superspeed.log"

cleanup() {
	rm -f "${SCRIPT_DIR}/speedtest.tgz" "${speedLog}" && rm -rf "${SCRIPT_DIR}/speedtest-cli"
}
...
checkspeedtest() {
	if [ ! -e "${SCRIPT_DIR}/speedtest-cli/speedtest" ]; then
		echo "正在安装 Speedtest-cli"
		mkdir -p "${SCRIPT_DIR}/speedtest-cli"
		case $(uname -m) in
			i?86) arch="i386" ;;
			x86_64) arch="x86_64" ;;
			armv5*) arch="armel" ;;
		...
		wget -qO "${SCRIPT_DIR}/speedtest.tgz" "https://install.speedtest.net/app/cli/ookla-speedtest-1.2.0-linux-${arch}.tgz"
		tar zxvf "${SCRIPT_DIR}/speedtest.tgz" -C "${SCRIPT_DIR}/speedtest-cli/" > /dev/null 2>&1 && chmod a+rx "${SCRIPT_DIR}/speedtest-cli/speedtest"
	fi
}
...
speed_test(){
	...
	local nodeISP=$3
	local strnodeLocation="${nodeLocation}　　　　　　"

	true > "$speedLog"
	"${SCRIPT_DIR}/speedtest-cli/speedtest" -p no -s "$1" --accept-license --accept-gdpr > "$speedLog" 2>&1
	local is_upload
	is_upload=$(grep 'Upload' "$speedLog")
	if [[ ${is_upload} ]]; then
		local REDownload reupload relatency temp
		REDownload=$(awk -F ' ' '/Download/{print $3}' "$speedLog")
		reupload=$(awk -F ' ' '/Upload/{print $3}' "$speedLog")
		relatency=$(grep 'Latency:' "$speedLog" | awk -F 'Latency: ' '{print $2}' | awk -F ' ' '{print $1}' | tr -d 'ms' | tr -d ',')
...
runtest() {
	...
	if [[ ${selection} == 1 ]]; then
		echo "——————————————————————————————————————————————————————————"
		printf "%-6s%s%s%-24s%s%-10s%s%-10s%-8s\n" "ID" "" "" "测速服务器信息" "" "上传/Mbps" "" "下载/Mbps" "延迟/ms"
		start=$(date +%s)
